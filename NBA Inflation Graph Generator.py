...
for year in range(first_year,second_year):
    ...
    NBA_data=pd.read_html(url)[0]  #, flavor='html5lib'
    ...
    for y in tuple(range(0,23)):
        ...
        if len(stats_list)>1: #the stat was actually recorded that year
            ...
            std_list.append(float(0.0))
            
        


    NBA_means[int(year)] = means_list
    NBA_STDs[int(year)] = std_list
...
VenD.index=['MP', 'FGM', 'FGA', 'FG%','3P', '3PA', '3P%', '2P', '2PA', '2P%', 'eFG%', 'FT', 'FTA', 'FT%', 'ORB',  'DRB',   'TRB',  'AST',  'STL',  'BLK',  'TOV',   'PF',   'PTS']
...
VenD["Years"]= range(first_year,second_year)
